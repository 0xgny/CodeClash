...
const GameRoom = ({ 
  ...
  // Problem bank with multiple LeetCode Easy problems
  const problems = [
    {
      ...
      validationKeywords: ["nums", "target", "return ["]
    },
    {
    ...
    },
    {
      ...
      validationKeywords: ["Set", "size", "length"]
    },
    {
      title: "Best Time to Buy and Sell Stock",
      difficulty: "EASY",
      description:
        "Given an array prices where prices[i] is the price of a given stock on the ith day, maximize profit by choosing a single day to buy one stock and choosing a different day to sell that stock in the future.",
      examples: [
      ...
      validationKeywords: ["min", "profit", "Math.max"]
    },
    {
      ...
      validationKeywords: ["current", "max", "Math.max"]
    },
    {
      title: "Plus One",
      difficulty: "EASY",
      ...
      validationKeywords: ["carry", "unshift", "return ["]
    }
  ];

  // Select one problem at random per session
  const [problem, setProblem] = useState(() => problems[Math.floor(Math.random() * problems.length)]);
  ...
  // Utility to check keyword coverage for validation/progress
  const getMatchedKeywordCount = (src) => {
    if (!problem || !problem.validationKeywords) return 0;
    const lowerSrc = src.toLowerCase();
    return problem.validationKeywords.reduce((count, keyword) => {
      return lowerSrc.includes(keyword.toLowerCase()) ? count + 1 : count;
    }, 0);
  };
  ...
  // Calculate my progress based on code and current problem
  useEffect(() => {
    const calculateProgress = () => {
      ...
      // Problem-aware scoring via keywords
      const matched = getMatchedKeywordCount(code);
      const totalKeywords = problem?.validationKeywords?.length || 0;
      if (totalKeywords > 0) {
        // up to 20 points for keyword coverage (besides complete detection below)
        const keywordScore = Math.min(Math.floor((matched / totalKeywords) * 20), 20);
        progress += keywordScore;
      }

      // Complete solution detection: all keywords present
      if (problem && totalKeywords > 0 && matched === totalKeywords) {
        progress = 100;
        if (myProgress < 100) {
  ...
  const handleRunTests = () => {
    ...
    setTimeout(() => {
      ...
      const matched = getMatchedKeywordCount(code);
      const totalKeywords = problem?.validationKeywords?.length || 0;
      const hasCompleteLogic = totalKeywords > 0 && matched === totalKeywords;
      
      if (hasCompleteLogic) {
        setTestResults({
          status: 'passed',
          passed: (problem?.testCases?.length || 0),
          total: (problem?.testCases?.length || 0),
          details: (problem?.testCases || []).map(tc => ({ input: tc.input, expected: tc.expected, actual: tc.expected, passed: true }))
        });
      } else if (hasBasicStructure) {
        const half = Math.max(1, Math.floor(((problem?.testCases?.length || 1)) / 2));
        const details = (problem?.testCases || []).map((tc, idx) => ({
          input: tc.input,
          expected: tc.expected,
          actual: idx < half ? tc.expected : 'Incorrect',
          passed: idx < half
        }));
        setTestResults({ status: 'partial', passed: details.filter(d => d.passed).length, total: details.length, details });
      } else {
        const details = (problem?.testCases || []).map(tc => ({ input: tc.input, expected: tc.expected, actual: 'Error', passed: false }));
        setTestResults({ status: 'failed', passed: 0, total: details.length, details });
      }
    }, 2000);
  };
